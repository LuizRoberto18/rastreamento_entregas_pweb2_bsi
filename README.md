...
persistencia com Prisma ORM e composicao de dependencias em ponto unico.

## Atividade 08 (Prisma ORM, migrations versionadas e filtros avancados)

Principais entregas:
- schema Prisma com models `Entrega`, `EventoEntrega`, `Motorista` e enums de status;
- migrations versionadas em `prisma/migrations` via `prisma migrate dev`;
- repositories de entregas e motoristas reimplementados com `PrismaClient`;
- `GET /api/entregas` com paginacao (`page`, `limit`) e filtro por intervalo de datas (`createdDe`, `createdAte`);
- seed em `prisma/seed.js` com dados de demonstracao.
